...
export function useComfyUI() {
  ...
  useEffect(() => {
    ...
    const handleServerChanged = () => {
      ...
      setWsConnected(false)
      wsAttemptCount.current = 0
      setTimeout(() => {
        checkConnectionFn()
      }, 500)
    }

    cm.on('ws:connected', handleWsConnected)
    cm.on('ws:disconnected', handleWsDisconnected)
    cm.on('health:result', handleHealthResult)
    cm.on('active:changed', handleServerChanged)
    cm.on('servers:changed', handleServerChanged)

    return () => {
      cm.off('ws:connected', handleWsConnected)
      cm.off('ws:disconnected', handleWsDisconnected)
      cm.off('health:result', handleHealthResult)
      cm.off('active:changed', handleServerChanged)
      cm.off('servers:changed', handleServerChanged)
    }
  }, [])
  ...
  const checkConnectionFn = useCallback(async () => {
    ...
    if (connected && !comfyui.isWebSocketConnected() && wsAttemptCount.current < 5) {
      try {
  ...
  useEffect(() => {
    checkConnectionRef.current = checkConnectionFn;
    checkConnectionFn();

    const interval = setInterval(checkConnectionFn, 15000);
